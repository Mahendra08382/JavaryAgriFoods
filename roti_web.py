...
products_data = [
    {
        ...
        "badge": "Bestseller",
        "badge_color": "#9c27b0",
        "image": r"C:\Users\2025\JavaryAgriFoods\images\roti_banner.jpg",
        "emoji": "🫓",
        "rating": 4.9,
        "reviews": 128,
    },
...
.nav-links a { text-decoration: none; color: #333 !important; font-weight: 600; font-size: 13px; text-transform: uppercase; letter-spacing: 0.5px; }
...
.hero::before { content: ''; position: absolute; top: -50%; right: -15%; width: 500px; height: 500px; background: rgba(255,255,255,0.04); border-radius: 50%; }
.hero-text { max-width: 520px; z-index: 2; }
.hero-badge { display: inline-block; background: #ff8f00; padding: 5px 16px; border-radius: 20px; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 18px; }
...
.hero-btn { background: #ff8f00; padding: 14px 34px; border-radius: 50px; text-decoration: none; font-weight: 700; font-size: 15px; box-shadow: 0 4px 14px rgba(255,143,0,0.4); display: inline-block; }
.hero-btn:hover { background: #e65100; transform: translateY(-3px); }
.hero-emoji { font-size: 150px; z-index: 2; animation: bob 3s ease-in-out infinite; filter: drop-shadow(0 10px 25px rgba(0,0,0,0.3)); }
@keyframes bob { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-18px); } }

.features { display: flex; justify-content: center; background: #ffffff !important; box-shadow: 0 3px 15px rgba(0,0,0,0.05); border-bottom: 1px solid #eee; }
.feat { display: flex; align-items: center; gap: 12px; padding: 22px 36px; border-right: 1px solid #f0f0f0; flex: 1; justify-content: center; }
.feat:last-child { border-right: none; }
.feat-icon { font-size: 28px; background: #e8f5e9; padding: 10px; border-radius: 50%; width: 50px; height: 50px; display: flex; align-items: center; justify-content: center; }
...
.sec-badge { display: inline-block; background: #e8f5e9; color: #2e7d32 !important; padding: 5px 18px; border-radius: 20px; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 12px; }
...
.p-card-top { background: #ffffff !important; border-radius: 16px 16px 0 0; border: 1px solid #e0e0e0; border-bottom: none; position: relative; margin-top: 15px; height: 10px; }
.p-badge-wrap { position: absolute; top: 8px; left: 10px; z-index: 10; }
.p-badge-tag { display: inline-block; padding: 4px 11px; border-radius: 5px; font-size: 10px; font-weight: 700; color: #ffffff !important; text-transform: uppercase; }
.p-emoji-area { height: 200px; width: 100%; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, #f8faf8, #e8f5e9); border-left: 1px solid #e0e0e0; border-right: 1px solid #e0e0e0; }
.p-emoji { font-size: 80px; }
.p-card-bottom { background: #ffffff !important; border-radius: 0 0 16px 16px; border: 1px solid #e0e0e0; border-top: none; padding: 18px; margin-bottom: 5px; }
...
.p-unit { display: inline-block; background: #f0f0f0; padding: 2px 9px; border-radius: 4px; font-size: 11px; color: #555 !important; margin-bottom: 10px; }
...
.p-off { background: #fff3e0; color: #ff8f00 !important; padding: 2px 7px; border-radius: 4px; font-size: 11px; font-weight: 700; }

[data-testid="stImage"] { border-left: 1px solid #e0e0e0; border-right: 1px solid #e0e0e0; overflow: hidden; margin: 0 !important; }
[data-testid="stImage"] img { height: 200px !important; object-fit: cover !important; border-radius: 0 !important; }

.why-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 22px; padding: 28px 50px 50px; }
.why-card { background: #ffffff !important; border-radius: 14px; padding: 30px 20px; text-align: center; border: 2px solid #f0f0f0; transition: all 0.35s; }
.why-card:hover { transform: translateY(-6px); box-shadow: 0 12px 30px rgba(0,0,0,0.07); border-color: #c8e6c9; }
...
.contact-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 25px; padding: 25px 50px 45px; }
.c-card { text-align: center; padding: 30px 20px; border-radius: 14px; border: 2px solid #f0f0f0; background: #ffffff !important; transition: all 0.3s; }
.c-card:hover { border-color: #2e7d32; box-shadow: 0 8px 25px rgba(46,125,50,0.08); transform: translateY(-4px); }
...
.footer-grid { display: grid; grid-template-columns: 2fr 1fr 1fr; gap: 40px; padding-bottom: 35px; border-bottom: 1px solid #333; }
...
.success-box { background: linear-gradient(135deg, #e8f5e9, #c8e6c9) !important; border: 2px solid #4caf50; border-radius: 20px; padding: 40px; text-align: center; margin: 20px 0; }
...
.empty-cart { text-align: center; padding: 40px; background: #fff8e1 !important; border-radius: 16px; border: 2px dashed #ffcc02; }
...
div.stFormSubmitButton > button {
    background: linear-gradient(135deg, #5cff6a, #2ecc71) !important;
...
div.stFormSubmitButton > button:active, div.stFormSubmitButton > button:focus { color: #000000 !important; }
div.stButton > button p, div.stButton > button span, div.stButton > button div,
div.stFormSubmitButton > button p, div.stFormSubmitButton > button span, div.stFormSubmitButton > button div { color: #000000 !important; }
...
<div class="sec-head" id="products">
    ...
    <p>Use + and − buttons to add items to your cart</p>
</div>
""", unsafe_allow_html=True)
...
for i, prod in enumerate(products_data):
    with cols[i % 3]:
        ...
        # ===== [ − ]  QTY  [ + ] =====
        btn_minus, qty_show, btn_plus = st.columns([1, 1, 1])

        with btn_minus:
            st.button(
                "➖",
                key=f"minus_{prod['name']}",
        ...
        with qty_show:
            ...
            st.markdown(
                f'<div class="qty-display {qty_class}">{current_qty}</div>',
                unsafe_allow_html=True
            )
        ...
        with btn_plus:
            st.button(
                "➕",
                key=f"plus_{prod['name']}",
                on_click=increase_qty,
                args=(prod["name"], prod["price"], prod["unit"]),
                use_container_width=True
            )
            ...
            if cust_name and cust_phone and cust_address:
                ...
                if ok:
                    ...
                    <div class="success-box">
                        <span class="check">✅</span><h2>Order Submitted!</h2>
                    ...
                    <a href="https://wa.me/{WHATSAPP_NUMBER}?text={enc}" target="_blank" style="
                        display:inline-flex;align-items:center;gap:10px;background:linear-gradient(135deg,#25d366,#128c7e);
                        color:#ffffff !important;padding:14px 35px;border-radius:50px;text-decoration:none;font-weight:700;
                        box-shadow:0 5px 18px rgba(37,211,102,0.35);">💬 Send via WhatsApp</a>
                    """, unsafe_allow_html=True)
            else:
                st.error("⚠️ Fill Name, Phone, Address.")
else:
    st.markdown("""
    <div class="empty-cart"><span style="font-size:50px;">🛒</span>
    <h3>Your cart is empty</h3><p>Use + button on any product above to add it</p></div>
    """, unsafe_allow_html=True)
